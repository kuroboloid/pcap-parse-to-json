...
/* ФУНКЦИИ */
	packet_t * packet_new(void);							// Новый пакет.				
	void packet_preprocess(const struct pcap_pkthdr *,		// Парсинг.
						   const u_char *, int pnum);
	void packet_to_json (packet_t *pkt);					// Запись в json-формат.
